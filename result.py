"""Calibration results: plots + metrics.

This script supersedes plot_calibration.py.
...
- Runs one simulation, aligns simulated pressures onto the observed time index.
- Writes plots under outputs/reports/plots.
- Prints calibration metrics (RMSE, R^2) overall and per-sensor.

Outputs
-------
- outputs/reports/plots/sensors_overlay.png
- outputs/reports/plots/<SENSOR_ID>.png
- outputs/reports/plots/residual_hist.png
- outputs/reports/plots/obs_vs_sim_scatter.png
"""
...
def _flatten_valid(obs: pd.DataFrame, sim: pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
    a = obs.to_numpy(dtype=float)
    b = sim.to_numpy(dtype=float)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Calibration results: plots + metrics")
    parser.add_argument(
        "--params-json",
    ...
    # Metrics (overall + per-sensor)
    y_true_all, y_pred_all = _flatten_valid(obs_aligned, sim_aligned)
    if len(y_true_all) == 0:
        raise ValueError("No finite overlapping samples after alignment.")

    overall_rmse = _rmse(y_true_all, y_pred_all)
    overall_r2 = _r2(y_true_all, y_pred_all)

    per_sensor = []
    for s in sensors:
        yt, yp = _flatten_valid(obs_aligned[[s]], sim_aligned[[s]])
        if len(yt) == 0:
            continue
        per_sensor.append({
            "sensor": s,
    ...
    print(f"  R^2:     {overall_r2:.6f}\n")

    if per_sensor:
        dfm = pd.DataFrame(per_sensor).sort_values("rmse", ascending=False)
        # Print a short table (worst 10 by RMSE)
        head = dfm.head(10)
        print("Per-sensor (worst RMSE):")
    ...
    residual = (sim_aligned - obs_aligned)
    res_vals = residual.to_numpy(dtype=float)
    res_vals = res_vals[np.isfinite(res_vals)]

    if len(res_vals):
        fig, ax = plt.subplots(figsize=(10, 4))
        ax.hist(res_vals, bins=50, alpha=0.85)
        ax.set_title("Residual histogram (simulated - observed)")
        ax.set_xlabel("Residual")
        ax.set_ylabel("Count")
        ax.grid(True, alpha=0.25)
        out = plot_dir / "residual_hist.png"
        fig.tight_layout()
        fig.savefig(out, dpi=220)
        plt.close(fig)
    ...
    ax.scatter(y_true_all, y_pred_all, s=6, alpha=0.25)
    lo = float(np.nanmin([np.min(y_true_all), np.min(y_pred_all)]))
    hi = float(np.nanmax([np.max(y_true_all), np.max(y_pred_all)]))
    ax.plot([lo, hi], [lo, hi], "k--", linewidth=1.0)
    ax.set_title("Observed vs Simulated (all sensors)")
    ax.set_xlabel("Observed")
    ax.set_ylabel("Simulated")
    ax.grid(True, alpha=0.25)
    out = plot_dir / "obs_vs_sim_scatter.png"
    fig.tight_layout()
    fig.savefig(out, dpi=220)
    plt.close(fig)
